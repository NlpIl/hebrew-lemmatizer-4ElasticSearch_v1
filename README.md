...
1.צריך לעשות clone למחשב האישי ,ולוודא שמותקן אצלנו JDK מומלץ להשתמש בגרסה 19 וגם gradle גרסה 8.4 כדאי לבנות הפלאגין stopwords ו lemmas כדאי לחבר אותם לElastic.אפשר להעזר בקובץ build.sh שניתן להריץ אחרי ההתקנה של JDK ו gradle והוא ירים שני קונטינרים dicta שאחראי על חלקות הטקסט למשפטים ולשלוח batches בגודל 16 -אפשר לשנות את הגודל בקובץ dicta/src/lemmatization.py- לפלאגין בקריאת POST ,והשני הוא es0 שהוא קונטינר שמריץ Elastic search עם single node -אפשר להשתמש ביותר מ node אחד אבל כל קריאת POST תתבצע פעמיים בגלל ה replica -,התקשורת עם Elastic נעשית דרך הפלגין שלנו שמעביר לו משפטים אחרי עיבוד במקום להעביר לו אות אות איך שהוא עובד בדרך כלל.
2. מצורף שני קבצי JSON שנתן להיעזר בהם בלתת ל index הגדרות להשתמש בפלאגין שלנו ברגע היצירה שלו שני הקבצים הם ,mappings_vectors_he.json ו settings_he.json , דוגמה לאיך להתשמש בהם בקובץ plugin_test.py.

הערות:
1. לוודא של git lfs מותקן לפני ה clone .
2.גודל JVM ב docker-compose.yml הוא 512m אבל אפשר להגדיל אותו עבור מחשבים חזקים , אחרת Elastic לא יעלה.
3. מודל dicta-tiny מבוסס על cpu לכן כלל שמקצים לו יותר cpu זמן העיבוד הולך וקטן .
